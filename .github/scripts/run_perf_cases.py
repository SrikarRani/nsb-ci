#!/usr/bin/env python3
"""Run a curated set of NSB perf cases and write a summary CSV."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run curated NSB perf cases")
    parser.add_argument("--perf-root", required=True, help="Path to the perf harness repo")
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Extra arg forwarded to run_nsb_perf_capture.sh; repeatable",
    )
...
def run_case(
    ...
    capture_script = perf_root / "scripts" / "run_nsb_perf_capture.sh"
    ...
    print(f"\n=== Curated perf case: nodes={nodes}, rate={rate} ===", flush=True)
    completed = subprocess.run(command, cwd=perf_root)
...
def main() -> int:
    ...
    rows = [
        run_case(perf_root, output_root, args.duration, nodes, rate, args.capture_arg)
        for nodes, rate in cases
    ]
    ...
    print(f"\nCurated perf suite complete. Summary CSV: {summary_csv}")
